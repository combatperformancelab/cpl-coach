...
const LEVELS = ["Loisir", "Amateur", "Amateur Ã©lite", "Semi-pro", "Pro"];
const SPORT_ICONS = { MMA: "ðŸ¥Š", Boxe: "ðŸ¥Š", Judo: "ðŸ¥‹", "Kick-boxing": "ðŸ¦µ", "Muay Thai": "ðŸ¥Š", Lutte: "ðŸ¤¼" };
...
export default function CoachApp({ user, onLogout }) {
  ...
  // Charger les clients en temps rÃ©el
  useEffect(() => {
  ...
  // Charger messages en temps rÃ©el
  useEffect(() => {
  ...
  const createClient = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "clients", uid), {
        ...
        program: { name: "Programme Ã  dÃ©finir", weeks: [], exercises: [] },
      });
      setShowAdd(false);
      setNewClient({ name: "", email: "", password: "", sport: "MMA", age: "", weight: "", category: "", level: "Amateur", goal: "" });
    } catch (e) {
      setAddError(e.code === "auth/email-already-in-use" ? "Cet email est dÃ©jÃ  utilisÃ©." : "Erreur : " + e.message);
  ...
  const addExercise = () => {
    const exercises = [...activeClient.program.exercises, { name: "Nouvel exercice", sets: "3x10", load: "â€”", note: "" }];
    setActiveClient({ ...activeClient, program: { ...activeClient.program, exercises } });
  };
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: `Tu es un assistant prÃ©parateur physique expert en sports de combat. AthlÃ¨te : ${c.name}, ${c.age}ans, ${c.weight}kg, ${c.sport} (${c.category}), niveau ${c.level}. Objectif : ${c.goal}. Programme : ${c.program?.name}. RÃ©ponds en franÃ§ais, concis et professionnel.`,
          messages: newHist,
        })
      });
    ...
    <div style={S.app}>
      ...
      <div style={S.header}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setShowAdd(true)} style={{ ...S.btn("#c9f"), fontSize: 18, padding: "6px 14px" }}>ï¼‹</button>
          <button onClick={onLogout} style={{ ...S.btn("#666"), padding: "6px 12px", fontSize: 12 }}>DÃ©co</button>
        </div>
      </div>

      <div style={{ padding: "16px 16px 0", display: "flex", gap: 10 }}>
        {[["ATHLÃˆTES", clients.length, "#c9f"], ["SÃ‰ANCES", clients.reduce((a, c) => a + (c.stats?.sessions || 0), 0), "#f4a261"], ["EN FORME", clients.filter(c => (c.stats?.notes?.at(-1) || 0) >= 7).length, "#2a9d8f"]].map(([l, v, col]) => (
      ...
      <div style={{ padding: "14px 16px" }}>
        <div style={{ fontSize: 11, color: "#555", letterSpacing: 2, marginBottom: 10, fontWeight: 700 }}>MES ATHLÃˆTES</div>
        {clients.length === 0 && <div style={{ color: "#444", textAlign: "center", padding: 32, fontSize: 14 }}>Aucun athlÃ¨te â€” appuie sur ï¼‹ pour en ajouter</div>}
        {clients.map(c => {
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    ...
                    <span style={S.badge(c.color || "#c9f")}>{SPORT_ICONS[c.sport] || "ðŸ¥Š"} {c.sport}</span>
                  </div>
                  <div style={{ fontSize: 12, color: "#666", marginTop: 2 }}>{c.category} Â· {c.weight}kg Â· {c.level}</div>
                  {total > 0 && (
      ...
      {showAdd && (
        <div style={{ position: "fixed", inset: 0, background: "#000b", display: "flex", alignItems: "flex-end", zIndex: 100 }} onClick={e => e.target === e.currentTarget && setShowAdd(false)}>
          <div style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "#13141c", borderRadius: "20px 20px 0 0", padding: "24px 20px 40px", border: "1px solid #2a2b3e", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ fontFamily: "'Oswald',sans-serif", fontSize: 18, color: "#c9f", letterSpacing: 2, marginBottom: 16 }}>NOUVEL ATHLÃˆTE</div>
            {[["Nom complet", "name", "text"], ["Email", "email", "email"], ["Mot de passe", "password", "password"], ["Ã‚ge", "age", "number"], ["Poids (kg)", "weight", "number"], ["CatÃ©gorie (ex: -77kg)", "category", "text"], ["Objectif", "goal", "text"]].map(([label, key, type]) => (
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              ...
              <button onClick={createClient} disabled={adding} style={{ ...S.btn("#c9f"), flex: 1, background: "#c9f22" }}>{adding ? "CrÃ©ation..." : "CrÃ©er le profil"}</button>
            </div>
          </div>
        </div>
      )}
  ...
  // â”€â”€ VUE CLIENT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const c = activeClient;
  return (
    <div style={{ ...S.app, display: "flex", flexDirection: "column", height: "100vh" }}>
      ...
      <div style={{ ...S.header, gap: 12, flexShrink: 0 }}>
        <button onClick={() => setView("dashboard")} style={{ background: "none", border: "none", color: "#c9f", fontSize: 22, cursor: "pointer", padding: 0 }}>â†</button>
        ...
        <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 15 }}>{c.name}</div><div style={{ fontSize: 11, color: "#666" }}>{c.sport} Â· {c.category}</div></div>
        <span style={S.badge(c.color || "#c9f")}>{c.stats?.sessions || 0} sÃ©ances</span>
      </div>

      <div style={{ display: "flex", borderBottom: "1px solid #1a1b26", background: "#0e0f16", flexShrink: 0 }}>
        {[["fiche", "ðŸ“‹ Fiche"], ["programme", "ðŸ’ª Prog."], ["messages", "ðŸ’¬ Msgs"], ["ia", "ðŸ¤– IA"]].map(([key, label]) => (
        ...
        {clientTab === "fiche" && (
          <div>
            <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
              {[["Ã‚ge", c.age + " ans", "#c9f"], ["Poids", c.weight + " kg", "#f4a261"], ["Sessions", c.stats?.sessions || 0, "#2a9d8f"]].map(([l, v, col]) => (
                <div key={l} style={S.statBox(col)}>
            ...
            <div style={{ background: "#13141c", border: "1px solid #1e1f2e", borderRadius: 14, padding: 16, marginBottom: 12 }}>
              ...
              {[["Objectif", c.goal], ["Niveau", c.level], ["Sport", c.sport], ["CatÃ©gorie", c.category], ["Suivi depuis", c.joinDate]].map(([k, v]) => (
            ...
            {c.stats?.poids?.length > 1 && (
              <div style={{ background: "#13141c", border: "1px solid #1e1f2e", borderRadius: 14, padding: 16 }}>
                <div style={{ fontSize: 11, color: "#555", letterSpacing: 2, marginBottom: 12, fontWeight: 700 }}>Ã‰VOLUTION POIDS</div>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              ...
              <button onClick={() => editProgram ? saveExercises() : setEditProgram(true)} style={S.btn(editProgram ? "#f4a261" : "#c9f")}>
                {editProgram ? "âœ“ Sauver" : "âœï¸ Modifier"}
              </button>
            </div>
              ...
              <div style={{ background: "#13141c", border: "1px solid #1e1f2e", borderRadius: 14, padding: 16, marginBottom: 14 }}>
                ...
                {c.program.weeks.map((w, i) => (
                  <div key={i} onClick={() => toggleWeek(i)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < c.program.weeks.length - 1 ? "1px solid #1a1b26" : "none", cursor: "pointer" }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: w.done ? (c.color || "#c9f") + "33" : "#1a1b26", border: `2px solid ${w.done ? (c.color || "#c9f") : "#2a2b3a"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>{w.done ? "âœ“" : "â—‹"}</div>
                    <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, color: w.done ? "#e8e8e8" : "#666" }}>{w.label} â€” {w.focus}</div></div>
                    <div style={S.badge(w.done ? (c.color || "#c9f") : "#444")}>{w.done ? "FAIT" : "Ã€ FAIRE"}</div>
                  </div>
                ))}
              </div>
            )}
            <div style={{ background: "#13141c", border: "1px solid #1e1f2e", borderRadius: 14, padding: 16 }}>
                  ...
                  {editProgram ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        <input value={ex.sets} onChange={e => updateExercise(i, "sets", e.target.value)} placeholder="SÃ©ries" style={{ ...S.input, flex: 1, fontSize: 12 }} />
              ...
              {editProgram && <button onClick={addExercise} style={{ ...S.btn("#c9f"), width: "100%", marginTop: 12, textAlign: "center" }}>ï¼‹ Ajouter exercice</button>}
            </div>
          </div>
        )}
        ...
        {clientTab === "messages" && (
          <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
            <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
              {messages.length === 0 && <div style={{ color: "#444", textAlign: "center", padding: 32, fontSize: 13 }}>Aucun message â€” commencez la conversation</div>}
              {messages.map(m => (
            ...
            <div style={{ padding: 12, borderTop: "1px solid #1a1b26", display: "flex", gap: 8, background: "#0e0f16" }}>
              ...
              <button onClick={sendMsg} style={{ ...S.btn(c.color || "#c9f"), padding: "10px 16px", fontSize: 16 }}>âž¤</button>
            </div>
          </div>
        )}
        ...
        {clientTab === "ia" && (
          <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
            <div style={{ padding: "12px 16px", background: "#13141c", borderBottom: "1px solid #1e1f2e", flexShrink: 0 }}>
              <div style={{ fontSize: 12, color: "#c9f", fontWeight: 700, letterSpacing: 1 }}>ðŸ¤– ASSISTANT IA â€” {c.name}</div>
              <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>Conseils personnalisÃ©s basÃ©s sur le profil de l'athlÃ¨te</div>
            </div>
            ...
            <div style={{ padding: 12, borderTop: "1px solid #1a1b26", display: "flex", gap: 8, background: "#0e0f16" }}>
              ...
              <button onClick={sendIa} disabled={iaLoading} style={{ ...S.btn("#c9f"), padding: "10px 16px", fontSize: 16 }}>âž¤</button>
            </div>
          </div>
        )}
      </div>
